...
ILE_WYN = 1000 # uruchomienia programu

PR_KRZYZ = 80 #[%] prawdopodobieństwo krzyżowania
PR_MUT = 10    #[%] prawdopodobieństwo mutacji

ILE_OS = 24   # ilość osobników
LB_POP = 6   # liczba populacji


def f(x, A = 4, B = 7, C = 2):
    return A * x * x + B * x + C

with open('sprawdzenie_parametrow.txt', 'w') as file:
    for mutacja in range(0, 16):
        for krzyzowanie in range(70, 101):
            for c in range(ILE_WYN):
                populacja = Populacja(ILE_OS, krzyzowanie, mutacja, f)
                ...
                file.write(f'{mutacja} {krzyzowanie} {wynik}\n')
